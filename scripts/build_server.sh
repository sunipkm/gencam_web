...
while test $# -gt 0; do
  case "$1" in
    -h|--help)
      echo "build_server.sh [--release] [--webgpu] [--open]"
      echo ""
...
FINAL_WASM_PATH=assets/${OUT_FILE_NAME}_bg.wasm
...
wasm-bindgen "${WASM_PATH}" --out-dir assets --out-name ${OUT_FILE_NAME} --no-modules --no-typescript
